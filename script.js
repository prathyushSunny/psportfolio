...
setTimeout(function () {
  ...
  const setMyAge = () => {
    const birthdate = new Date("1998-01-05");
    const ageInMilliseconds = currentDate - birthdate;
    const ageInYears = ageInMilliseconds / (365 * 24 * 60 * 60 * 1000);
    const roundedAge = Math.floor(ageInYears);
    document.querySelector(".my-age-num").textContent = roundedAge;
  };
  const setMyExperience = () => {
    let experience = "";
  ...
  window.addEventListener("scroll", () => {
    ...
    if (
      window.pageYOffset > presentYOffset &&
      !hamburger.classList.contains("cross")
    )
